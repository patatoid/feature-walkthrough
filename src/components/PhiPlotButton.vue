    ...
    <div class="phi-overlay" v-if="isOpen" @click.self="closePlot()" @keydown.esc="closePlot()">
      <section class="ui segment phi-popover" role="dialog" aria-modal="true" :aria-labelledby="titleId">
        ...
        <template v-else-if="report">
          <div class="phi-stats">
            <span><strong>{{ report.examples }}</strong> examples</span>
          ...
          <p class="phi-note">Trained locally in WebAssembly. Each child node is a response to its parent, with ancestry context decaying by 0.65 toward the root.</p>
        </template>
      </section>
    </div>
  </span>
